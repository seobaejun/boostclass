...
class PlaywrightMCPServer {
  ...
  private state: PlaywrightState = { browserType: 'chromium' };
  ...
  private setupToolHandlers(): void {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        switch (name) {
          case 'launch_browser':
            return await this.launchBrowser(args as any);
          
          case 'navigate_to':
            return await this.navigateTo(args as any);
          ...
          case 'fill_input':
            return await this.fillInput(args as any);
          
          case 'get_text':
            return await this.getText(args as any);
          
          case 'wait_for_element':
            return await this.waitForElement(args as any);
          
          case 'run_test':
            return await this.runTest(args as any);
  ...
  private async launchBrowser(args: { browserType?: 'chromium' | 'firefox' | 'webkit', headless?: boolean }): Promise<{ content: Array<{ type: string, text: string }> }> {
  ...
  private async navigateTo(args: { url: string }): Promise<{ content: Array<{ type: string, text: string }> }> {
    if (!this.state.page) {
  ...
  private async takeScreenshot(args: { path?: string, fullPage?: boolean }): Promise<{ content: Array<{ type: string, text: string }> }> {
    if (!this.state.page) {
  ...
  private async clickElement(args: { selector: string }): Promise<{ content: Array<{ type: string, text: string }> }> {
    if (!this.state.page) {
      throw new Error('브라우저가 시작되지 않았습니다.');
    }
  ...
  private async fillInput(args: { selector: string, text: string }): Promise<{ content: Array<{ type: string, text: string }> }> {
    if (!this.state.page) {
      throw new Error('브라우저가 시작되지 않았습니다.');
    }
  ...
  private async getText(args: { selector: string }): Promise<{ content: Array<{ type: string, text: string }> }> {
    if (!this.state.page) {
      throw new Error('브라우저가 시작되지 않았습니다.');
    }
  ...
  private async waitForElement(args: { selector: string, timeout?: number }): Promise<{ content: Array<{ type: string, text: string }> }> {
    if (!this.state.page) {
      throw new Error('브라우저가 시작되지 않았습니다.');
    }
  ...
  private async runTest(args: { testFile?: string, grep?: string }): Promise<{ content: Array<{ type: string, text: string }> }> {
  ...
  private async closeBrowser(): Promise<{ content: Array<{ type: string, text: string }> }> {
    if (this.state.browser) {
      await this.state.browser.close();
